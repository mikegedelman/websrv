...
int wrecv(int sockfd, char *buf, size_t len, int flags) {
   if (threading)
      return tp_recv(sockfd, buf, len, flags, request_timeout);
   else
      return recv_msg(sockfd, buf, len, flags);
}

int wsend(int sockfd, char *buf, size_t len, int flags) {
   if (threading)
      return tp_send(sockfd, buf, len, flags);
   else
      return send_msg(sockfd, buf, len, flags);
}
...
/* block (read) on given fd; 0 for timeout; -1 for error */
   ...
   if (threading) {
      ...
      for (;;) {
         if ((t = accept4(s,NULL,NULL,SOCK_NONBLOCK)) < 0) {
            ...
            tp_activate(serve_request, (void*)a);
            puts("thread started");
         }
      }
   }
...
/*
 * Blocking send/recv functions.
 */
int send_msg(int t, const char *c, int len, int flags) {
   if (c == NULL)
      return -1;
...
int recv_msg(int t, char *buf, int bufsize, int flags) {
   int bytes_recv = 0;
   
   while (bytes_recv < 1) 
      if ( (bytes_recv += recv(t,buf+bytes_recv,bufsize,flags)) < 0 ) {
         perror("recv");
         return -1;
      }
...
int recv_request(int sock) {
   ...
   int bytes_recvd = 0;
   while (1) {
      int bytes = wrecv(sock, buf_ptr, REQUEST_BUF_SIZE, 0);
      if (bytes < 0)
         return -1;
      else if (bytes == 0) {
   ...
   char req[8];
   sscanf(recv_buffer, "%s", req);

   if ( strcmp(req, "GET") == 0)
      return get_request(sock, recv_buffer+4);
   else {
      if ( !strcmp(req, "OPTIONS") || !strcmp(req,"HEAD") || 
           !strcmp(req,"POST") || !strcmp(req, "PUT") ||
           !strcmp(req, "DELETE") || !strcmp(req, "TRACE") ||
           !strcmp(req, "CONNECT"))
         err_response(sock, 501, "Not Implemented");
         return 0;
   }
...
void respond(int t, struct response *r, struct request_options *opt) {
   ...
   if (r->message_body) {
      wsend(t, buf, strlen(buf), MSG_MORE);
      wsend(t, r->message_body, r->content_length, 0);
   }
   else 
      wsend(t, buf, strlen(buf), 0);
...
int get_request(int t, char *req) {
   ...
   if (ending == NULL)
      ...
      get_cgi_file(t, file, &opt, query_string);
...

{
   ...
   if ((wsend(t, header_buf, strlen(header_buf), MSG_MORE)) < 0)
      perror("wsend");
   ...
   while (bytes_sent < sbuf.st_size) {
      ...
      bytes_sent += wsend(t, src, servsz, MSG_MORE);
      munmap(src, servsz);
   }
   wsend(t, "\r\n", 3, 0);

...
/* convenient way to construct a response */
struct response {
...
/* this isn't crucial right now, but can be expanded to handle all of the
 * headers that the client sends and then act accordingly 
 */
...
/* This server forks itself after establishing a connection with a remote
 * host. Then it waits for a request in recv_request
 */
int main(int argc, char **argv) {
   ...
   myport = (int) strtol(argv[1], (char**)NULL, 10);
...
/* below are simple wrappers for send/recv that make sure we send everything,
 * and at least recieve something.
 */

int send_loop(int t, const char *c, size_t len, int flags) {
   if (c == NULL)
      return -1;
...
/* convenient way to send an error response with a simple 
 * body indicating the error, and then exit child process 
 */
...
/* crlf - carriage return line feed - standard way of indicating newline
 * in HTTP. this finds two of them, which indicates the end of the request
 */
...
/*
 * will block until it gets a crlf. in the future, a timeout should be 
 * added to this so that we don't have idle connections forever
 */
int recv_request(int sock) {
   ...
   while (1) {
      int bytes = recv(sock, buf_ptr, REQUEST_BUF_SIZE, 0);
      if (bytes < 0) {
         perror("recv");
         return -1;
      }
   ...
   if ( strcmp(req, "GET") == 0)
      get_request(sock, recv_buffer+4);
   else {
      bad_request(sock);
   }
...
/*
 * builds a response header into buf based on response r
...
void respond(int t, struct response *r, struct request_options *opt) {
   ...
   if (r->message_body) {
      send_loop(t, buf, strlen(buf), MSG_MORE);
      send_loop(t, r->message_body, r->content_length, 0);
   }
   else 
      send_loop(t, buf, strlen(buf), 0);
 ...
 *   Host
 * for the time being we're not being strict about Host field and not 
 * supporting connection:keep-alive quite yet.
 */
...

{
   ...
   while (bytes_sent < sbuf.st_size) {
      ...
      bytes_sent += send(t, src, servsz, MSG_MORE);
      munmap(src, servsz);
   }
   send_loop(t, "\r\n", 3, 0);
}
